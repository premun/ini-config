...
namespace Config.IniFiles.Errors
{
    ...
    /// </summary>
    public sealed class InvalidValueError : FormatError
    {
        /// <summary>
        ...
        public InvalidValueError(string optionName, string value,
            OptionSpecifier formatSpecifier, int line)
            : base(
                "Option '" + optionName + "' could not be parsed as " +
                formatSpecifier.GetType()
                    .Name.Replace("OptionSpecifier", "")
                    .ToLower() + " (value: '" + value + "')", line)
        {
        }
    }
}