...
namespace Config.IniFiles
{
    ...
    {
        /// <summary>
        ///     The option identifier pattern
        /// </summary>
        private const string IdentifierPattern =
            @"[a-zA-Z\.\$:][a-zA-Z0-9_ \-\.:\$]*";

        /// <summary>
        ///     The reference pattern ${id#id}
        /// </summary>
        ...
        /// <returns>
        ///     Built IConfig filled with data
        /// </returns>
        public IConfig Build(ConfigFormatSpecifier formatSpecifier = null,
        ...
        /// <summary>
        ///     Gets a list of errors that occurred during building.
        /// </summary>
        /// <exception cref="System.InvalidOperationException">Getter for Errors called, but Build() not called before</exception>
            ...
            get
            {
                if (_errors == null)
                {
                    throw new InvalidOperationException(
                        "Getter for Errors called, but Build() not called before");
                }
        ...
        private void ParseToken(SectionHeaderToken token)
        {
            ...
            {
                ReportError(new DuplicateSectionError(sectionName,
                    _parser.GetLine()));
                return;
            }
        ...
        private void ParseToken(OptionToken token)
        {
            ...
            {
                ReportError(new InvalidIdentifierError(token.Name,
                    _parser.GetLine()));
                return;
            }
        ...
        private Option ParseOptionValue(OptionToken token, string sectionName)
        {
            ...
            if (regex.Success)
            {
                return new ReferenceOption(regex.Groups[1].Value,
                    regex.Groups[2].Value, _config);
            }

            // Checks if string is unknown
            ...
            {
                ReportError(new InvalidValueError(token.Name, token.Value,
                    formatOption, _parser.GetLine()));
                return new StringOption(token);
            }
        }
